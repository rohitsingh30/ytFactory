...
class RewriteContract:
    ...
    def regen_prompt(
        ...
        return (
            ...
            f"parts. CRITICAL: when shortening to fix `long_narration`, "
            f"DO NOT weaken the hook — the first 8 words MUST still "
            f"start with a question, an \"Am I wrong\"-frame, a claim "
            f"verb (refused, told, caught, found, threw, watched, "
            f"served, paid, lied), or a subject-verb opener like \"I + "
            f"verb\" / \"my + relationship + verb\". Cut from the "
            f"middle (setup, antagonist's response, your reasoning), "
            f"never from the hook line.\n\n"
