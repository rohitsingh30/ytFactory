"""AI Recap rewriter — turns one ``RawStory`` into a 60s tech-recap script.
...
- ``rewrite.py`` is AITA-tuned (first-person Reddit drama, hook patterns
  ...
  AI Recap is third-person factual tech news — different hook patterns,
...
- Keeping the prompt scoped to one channel makes it small and easy to
  ...
  variant that wants the same shape.
...
scrollpulse/variants/ai_recap.yaml`` consumes AI Recap scripts unchanged.
"""
...
class RecapRewriteError(RuntimeError):
    pass
...
def rewrite(raw_story: dict, *, model: str | None = None) -> dict:
    ...
    if not story_text:
        raise RecapRewriteError("raw_story has no title or body")

    prompt = _AI_RECAP_PROMPT.replace("__STORY__", story_text[:6000])
    print(f"[ai_recap_rewrite] authoring narration via claude CLI for {raw_story.get('slug')!r}…")
    ...
    if not hook or not narration or not titles:
        raise RecapRewriteError(f"missing required field in LLM output: {out!r}")
    ...
    if wc < 130 or wc > 175:
        ...
        print(
            f"[ai_recap_rewrite] WARNING: narration is {wc} words "
    ...
    if "FOLLOW for daily AI recaps" not in narration or "LIKE if this saved you a tab" not in narration:
        print(
            "[ai_recap_rewrite] WARNING: narration is missing the canonical CTA "
            "('FOLLOW for daily AI recaps. LIKE if this saved you a tab.'). "
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Author one ScrollPulse AI Recap variant script.")
    ap.add_argument(
        "--raw",
        required=True,
        help="Path to a raw story JSON (scrollpulse/ai_recap/raw/<slug>.json)",
    )
    ap.add_argument(
        "--out",
        default="scrollpulse/ai_recap/narrations",
    ...
    except (RecapRewriteError, llm.ClaudeCLIError) as e:
