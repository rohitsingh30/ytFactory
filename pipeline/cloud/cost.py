...
3. The service account running this code (laptop user creds in dev,
   the Cloud Run JOB SA in prod) needs ``roles/bigquery.dataViewer``
   on that dataset.
...
def fetch_cost_window(
    ...
    *,
    ...
    ``billing_account_id`` defaults to ``BILLING_ACCOUNT_ID`` env. The
    one for ``ytfactory-prod-v2`` is documented in
    ``docs/cloudrun_tts.md`` (``012E39-E4ECEB-7F119F``).
    """
    ...
    bid = billing_account_id or os.environ.get("BILLING_ACCOUNT_ID") or "012E39-E4ECEB-7F119F"
    if not bid:
