# Lazy re-export of the public API from submodules.
#
...
# (``pipeline.upload.upload`` / ``pipeline.upload.x_upload``) take
# effect via the package binding too. A static ``from`` import would
# capture the function object at import time and any later rebind on
# the submodule would be invisible through the package namespace.
#
...
from pipeline.upload import x_upload as _x_upload_mod
...
_X_PUBLIC = {
    "XUploadError": "XUploadError",
    "x_post_short": "post_short",  # exported under a different name
    "x_post": "x_post",
}
...
def __getattr__(name: str):
    ...
    if name in _X_PUBLIC:
        return getattr(_x_upload_mod, _X_PUBLIC[name])
    raise AttributeError(f"module 'pipeline.upload' has no attribute {name!r}")


def __dir__() -> list[str]:
    return sorted(set(globals()) | set(_UPLOAD_PUBLIC) | set(_X_PUBLIC))
