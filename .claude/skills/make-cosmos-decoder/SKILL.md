...
description: Author a "How We Knew" physics+space deep-dive for the Cosmos Decoded channel — pairs a specific physics prediction with the specific experiment, mission, or observation that confirmed it. Three-act structure (prediction → experiment → consequence) over 25-40 min long-form (16:9) PLUS a paired 50-60s Short (9:16) that isolates the act-2 measurement moment. Footage-only sourcing (NASA Image Library / NTRS / ESA / CERN / LIGO / Wikimedia / arXiv / LoC / Smithsonian / stock — NO AI image gen, NEVER YouTube news re-uploads). Produces narrations/<slug>.json + shotlist/<slug>.json (long-form) AND narrations/<slug>-short.json + shotlist/<slug>-short.json (Short), then hands off to scripts/historyrecapped/render_long_form.py and scripts/historyrecapped/render_footage_only.py — both channel-parametric on --channel cosmosdecoded. Use when the user says "make me a cosmos decoder", "how we knew <X>", "physics decoder on <Y>", "long-form physics", "Eddington 1919 video", "LIGO video", "JWST decoder". For 50-60s science-news Shorts (no long-form pairing) use /make-script. For Top-10 science countdowns use /make-top10. For sports docs use /make-sports-doc.
learnings_consulted:
