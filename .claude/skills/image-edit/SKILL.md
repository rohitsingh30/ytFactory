...
name: image-edit
description: >-
  Exploit FLUX.2 klein's image-to-image + multi-reference editing modes (we own them on cloud and currently throw them away — all 16 channel YAMLs only use plain T2I). Three jobs: (1) Cast-appearance lock — given a face image + shotlist, regenerate every shot with that face locked, fixing cast-drift per docs/cast_appearance_lock.md without retraining a LoRA; (2) Thumbnail variants — generate N stylistic variants of a rendered shot for A/B testing; (3) Background swap — regenerate the same character/composition on a new background for series continuity. All three call cloud/image-flux2-klein's /edit endpoint. Use when the user says "image-edit", "/image-edit", "lock the face", "regenerate shot with that character", "thumbnail variants", "swap the background", "use flux image-to-image", "multi-reference edit", "cast lock with flux", or after a render where cast drift is visible per /critique-video. For pure T2I image gen use the channel's existing image_provider route.
---
