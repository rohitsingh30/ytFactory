...
description: Batch-render N Shorts in parallel across the available cloud GPU capacity (each L4-backed Cloud Run service supports `max-instances=2`, so we can render 2 Shorts per service in parallel — today we render serial and lock one channel at a time). Wraps the existing `pipeline/render/shorts.py` (and `long_form.py`, `footage_only.py`, `sports_doc.py`) entry points with a `concurrent.futures.ThreadPoolExecutor` whose pool size is computed from `GET /api/cloud/health`'s observed max-instances per service. Pre-warms every required service first via `POST /api/cloud/warm` (or `cloud/warm_{tts,image}_services.sh` directly), then dispatches the batch, then collates per-render success/failure into a single report. Critical-path engineering wins: a 5-Short batch that today takes ~50 min (10 min × 5 serial) finishes in ~25 min (10 min × ⌈5/2⌉ pairs). Use when the user says "parallel render", "/parallel-render", "render N shorts at once", "batch render", "ship the daily 5", "render the queue", "parallel ship", "stop rendering serial", or has a queue of ≥2 ready-to-render shotlists. For single-render use the existing `pipeline/render/*.py` entry directly. For pre-warm only hit `POST /api/cloud/warm`. For cost projections of a parallel batch see the `/app/cloud` Cost section.
---
