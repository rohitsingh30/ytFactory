...
def _find_cast_path(slug: str) -> Path | None:
    """Locate data/intermediate/<channel>/cast/<slug>.json if it exists.

    The channel directory is unknown at make_short time, so we search
    every channel that has a cast/ subdir for this slug.
    """
    base = Path("data/intermediate")
    if not base.exists():
        return None
    for chan_dir in base.iterdir():
        cp = chan_dir / "cast" / f"{slug}.json"
        if cp.exists():
            return cp
    return None
...
def _load_forced_narration_lines(slug: str) -> list[str] | None:
    ...
    if not base.exists():
        return None
    for chan_dir in base.iterdir():
        sp = chan_dir / "shotlist" / f"{slug}.json"
        if not sp.exists():
            continue
        try:
...
def _find_voice_path(slug: str) -> Path | None:
    ...
    Mirrors _find_cast_path. Produced by clone_voice.py from a YouTube
    URL; presence forces tts_provider=f5_tts for this slug.
    """
    base = Path("data/intermediate")
    if not base.exists():
        return None
    for chan_dir in base.iterdir():
        vp = chan_dir / "voices" / f"{slug}.json"
        if vp.exists():
            return vp
    return None


def _voice_fingerprint(cfg: dict, pronunciation_dict: dict | None = None) -> dict:
...
def _find_script_path(slug: str) -> Path | None:
    """Locate data/intermediate/<channel>/scripts/<slug>.json if it exists.

    Same scan pattern as _find_cast_path — channel dir is unknown at
    make_short time, so we search every channel dir.
    """
    base = Path("data/intermediate")
    if not base.exists():
        return None
...
def _load_pronunciation_dict(slug: str) -> dict:
    ...
    if not base.exists():
        return {}
    for chan_dir in base.iterdir():
        dp = chan_dir / "dossier" / f"{slug}.json"
        if dp.exists():
            try:
...
def make_short(
    ...
    if cast_path is None and source_story:
        ...
        guessed_channel = channel_path.stem  # e.g. 'aita_animated'
        autopath = (
            Path("data/intermediate") / guessed_channel / "cast" / f"{slug}.json"
        )
        try:
    ...
    fp_now = _voice_fingerprint(cfg, pronunciation_dict=pronunciation_dict)
    ...
    if not audio_path.exists():
        ...
        audio.synthesize(
            ...
            # exclamation faster, ellipsis-trail slower).
...
def main() -> None:
    ...
    ap.add_argument("--channel", default="channels/aita_animated.yaml")
