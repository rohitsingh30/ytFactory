...
**How to apply:**
- Channel YAML: `historyrecapped/config.yaml`. Display name `History Recapped`. `render_style: footage_only` routes through `scripts/historyrecapped/render_footage_only.py` instead of image-gen. TTS (Shorts): Kokoro `am_michael` at speed 1.0 (zero-API local). Long-form sleep mode runs F5-TTS-MLX with sarah.wav. Image gen config (`z_image_turbo` 768x1344) is legacy from the abandoned hybrid format — kept in the YAML so a future hybrid Short doesn't break, but production renders skip image-gen entirely. Closer panel: `SUBSCRIBE for more deep dives / LIKE if you learned something`. Upload account `historyrecapped`, category 27 (Education).
