...
def readyz(request: Request) -> dict:
    _track_request_event("tts.readyz", request, {"endpoint": "/readyz", "model": "chatterbox"})
    t0 = time.time()
    _model()
...
def synth(req: SynthIn, request: Request) -> JSONResponse:
    _track_request_event(
        "tts.server",
        request,
        {"endpoint": "/synth", "model": "chatterbox", "text_chars": len(req.text or "")},
    )
