...
Two entry points:
    ...
    .venv/bin/python pull_backgrounds.py --query "asmr cooking no talking no face"
    .venv/bin/python pull_backgrounds.py --url <youtube-url>
...
``assets/cooking_loops/``. With ``--check-faces`` each candidate clip
...
def main() -> None:
    ...
    ap.add_argument("--out", default="assets/cooking_loops", help="Where to drop the 1080x1920 mp4 loops")
