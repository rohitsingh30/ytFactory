...
| 9 | SSE/live for `/api/queue` (currently 2 s poll) | Significant backend change — needs an SSE endpoint on FastAPI + new client subscription. Keep the 2 s poll for now; the SWR cache means tab-switches paint instantly even at that cadence. |
