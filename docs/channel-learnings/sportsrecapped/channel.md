...
- **Long-form documentary 20-30 min** (NEW 2026-05-04) — `/make-sports-doc`. Real broadcast match footage + commentator/YouTuber talking-head clips + b-roll + chapter cards + lower-thirds. 16:9 horizontal. Same OAuth as Shorts; separate `long_form_doc:` config block. Helpers: `sportstoriesanimated/scripts/{find_match_clips,find_commentary_takes,find_b_roll}.py` + render at `sportstoriesanimated/scripts/render_long_form_doc.py`. See [long_form_doc_format.md](long_form_doc_format.md).
