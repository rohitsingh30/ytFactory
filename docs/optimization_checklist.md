...
**Net stack:** 4 GPU services (z-image-turbo + chatterbox + indicf5 + asr-whisper) + render-worker + web. Optional editing-agent and clone-video-worker are off by default.
...
| Optimization | Detail | Saving |
|---|---|---|
| 5.1 | **GCS Fuse mount for weights** (not baked into Docker images) | Avoids 20+ GB Docker layer push stalls |
...
| Optimization | Detail |
|---|---|
| 8.1 | **Provider dispatch wired for all needed models** (`pipeline/images/images.py:_generate_impl`) | Z-Image-Turbo is the single production model |
...
| Optimization | Detail |
|---|---|
| 11.1 | **Default `image_provider: cloudrun_z_image_turbo`** in all channel YAMLs | One image service for all channels — simplifies infra |
...
`bash scripts/bootstrap_new_project.sh` on a fresh GCP account is one-shot, ~50 min, ~$3-4.
