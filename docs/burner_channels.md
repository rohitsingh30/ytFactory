...
| **Chrome windows opening unexpectedly during unrelated work** | The launchd-managed `pipeline.laptop_agent` long-polls the cloud control plane and spawns `burner_engage` workers that launch fresh-profile Chrome on their own | `tail /tmp/ytfactory-laptop-agent.err` — every spawn logs `executing task <id> kind=burner_engage`. To stop **for this session**: `launchctl unload ~/Library/LaunchAgents/com.ytfactory.laptop-agent.plist`. To **also persist across login/reboot** (otherwise launchd reloads it on next user session even though `KeepAlive` was just unloaded): `launchctl disable gui/$(id -u)/com.ytfactory.laptop-agent`. Then sweep orphan PIDs: `ps -ef \| grep -E "burner_engage\|create_burner_channel\|playwright/driver"` and `kill <PID>` each. Full runbook: [`docs/laptop_agent_cloud_contract.md` § "Operator kill-switch — how to stop the agent"](laptop_agent_cloud_contract.md#operator-kill-switch--how-to-stop-the-agent-2026-05-12). NOT caused by code that's "currently running" — the agent is autonomous. (Surfaced 2026-05-10; kill-switch ritual hardened 2026-05-12.) |
